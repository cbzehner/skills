...
name: how-it-works
description: >-
  Build a polished on-site or standalone how-it-works explainer for a codebase.
  Use for architecture explainers, HN-shareable internals pages, build journals,
  or standalone pages that need real source evidence, diagrams, and visual QA.
license: MIT
effort: high
...
Build an explainer page that turns a codebase into a source-grounded technical
reference in the site's own aesthetic. Prefer the smallest artifact that answers
the user's publishing goal.
...
4. **Evidence** — build command, browser screenshot, rendered page check, link
   check, or other proof required before calling the page done.
5. **Budget** — lightweight standalone artifact, same-repo page, or full polished
   publication with diagrams and visual QA.

If any of these are unclear, ask once with multiple-choice. Don't guess for #2 — it shapes the whole build.

## The Workflow

Use the minimum path that satisfies the page contract. Compress phases for small
standalone explainers; run the full path for polished published pages.

### Phase 1 — Frame Scope

Decide and write down:

- **Section list** (target 3–7). Lead the user toward concrete *micro-systems* (one specific mechanism per section) rather than abstract topics. "Cache and serving" beats "Performance"; "PDF parsing" beats "Data layer".
...
- **Diagrams**: hand-authored SVG / build-time Mermaid / mixed / none. Default:
  one diagram for every section that materially benefits from a visual model.
...
**Exit**: lightweight scope note. Commit a design spec only when the repo's
convention or the user asks for one.

### Phase 2 — Source the content (~20 min)

Two tasks, parallel only when they are genuinely independent:
...
3. **Diagram shortcode/component** only if diagrams are in scope. Render SVGs
   from the site's asset directory. See `references/diagrams.md` for the
   **critical SVG-rendering pitfall** — most HTML minifiers lowercase `viewBox`,
   breaking inlined SVGs. Use `<img src=...>` if the site minifies HTML.
...
**Exit**: site builds. Page renders at `/how-it-works/` or the agreed route.
Scoped diagrams are visible. TOC works when present. Build commands pass (`zola
check`, `hugo`, `next build`, etc.) and relevant tests are unaffected.
...
- One hero diagram per visual section. Each diagram tells the section's story without reading the prose.
- Ship as static files in `static/diagrams/how-it-works/`. Include `<title>` and `<desc>` for screen readers.

**Exit**: SVG files render correctly in the browser, file sizes are sensible, and
a screenshot of the page looks like the site.
...
At completion, the scoped deliverables exist and pass:

- [ ] Scope note or design spec exists when warranted
- [ ] `/how-it-works/` page (or equivalent route) built and routable
- [ ] Scoped diagrams render correctly in the browser
- [ ] TOC works if included
- [ ] Navigation entry points exist if this is an on-site page
