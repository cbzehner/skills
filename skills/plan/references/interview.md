---
name: plan-interview
description: Interview the user one question at a time about an existing plan or design, walking down each branch of the decision tree. Use when user says "grill me", "interview me", "push back on me", "ask me questions", or wants the assistant to probe their reasoning conversationally. Distinct from `counsel --adversarial`, which delivers a verdict via analysis. Do NOT use for premortem, failure-mode analysis, or stress-testing claims — those are `counsel --adversarial`.
license: MIT
effort: low
...
- **Exploring requirements from scratch** → stay in `plan --interview` and start from intent. This mode starts from a draft.
