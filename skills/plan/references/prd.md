...
- **Implementation plan from an existing PRD** → use `plan --from-prd`. PRDs describe what to build; plans describe phased execution.
...
If the interview gets long, stay in `plan --interview` and continue one question at a time.
...
After filing, share the issue URL or file path with the user. Suggest `plan --from-prd` as the next step.
