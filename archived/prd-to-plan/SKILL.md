---
name: prd-to-plan
description: Turn a PRD into a multi-phase implementation plan using tracer-bullet vertical slices, saved as a ralph-compatible plan file in ./plans/. Use when user has a PRD and wants to break it into phases, create an implementation plan, plan tracer bullets, or move from spec to executable plan. Distinct from superpowers:writing-plans — this consumes a PRD and produces phases that compose with the ralph skill.
license: MIT
effort: medium
...
Break a PRD into a phased implementation plan using vertical slices (tracer bullets). Output is a Markdown file in `./plans/` formatted for the `ralph` skill to execute.

## When NOT to Use

- **No PRD exists** → use `write-a-prd` first.
- **Generic plan-writing without a PRD** → use `superpowers:writing-plans`.
- **Direct execution of an existing plan** → use `ralph`.
...
**Format**: ralph-compatible. YAML frontmatter with status/progress fields, then `## Phase N` headings (which ralph treats as work units).
...
After writing, share the file path and suggest `ralph plans/<feature-slug>.md` as the next step.
